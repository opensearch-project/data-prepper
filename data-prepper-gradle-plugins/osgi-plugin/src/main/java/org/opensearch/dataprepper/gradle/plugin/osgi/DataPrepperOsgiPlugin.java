...
public class DataPrepperOsgiPlugin implements Plugin<Project> {
     ...
     */
    public static final String PLUGIN_ID = "org.opensearch.dataprepper.plugin";
     ...
     * <p>
     * This pattern and {@link #toOsgiVersion(String)} are intentionally duplicated in
     * {@code org.opensearch.dataprepper.plugin.osgi.DataPrepperOsgiPackages} in the
     * {@code plugin-framework-osgi} project. This project is an included (composite) build, so
     * neither side can depend on the other; the build-time manifest generator and the runtime host
     * must agree on version normalization, so change both together.
     */
    ...
    private void configureBndBundle(final Project project, final Logger logger) {
        ...
        final List<String> packageGlobs = toPackageGlobs(pluginClasses);

        // Build the bnd instructions string.
        // Export-Package is limited to the plugin's own packages (avoids fat-jar behavior).
        // bnd directives apply per clause, so -noimport must be attached to EVERY exported clause;
        // appending it once to the joined string would only affect the final package.
        // Import-Package uses bnd's default bytecode analysis to compute dependencies.
        final String exportClauses = packageGlobs.stream()
                .map(glob -> glob + ";-noimport:=true")
                .collect(Collectors.joining(","));
        final String privateClauses = String.join(",", packageGlobs);
        ...
        bndInstructions.append("Export-Package: !*.internal.*,").append(exportClauses).append('\n');
        bndInstructions.append("Import-Package: *").append('\n');
        bndInstructions.append("Private-Package: ").append(privateClauses).append('\n');

        if (pluginClasses != null && !pluginClasses.isEmpty()) {
            bndInstructions.append("Bundle-Activator: ").append(BUNDLE_ACTIVATOR_CLASS).append('\n');
            bndInstructions.append("DataPrepper-Plugin-Classes: ").append(normalizePackageList(pluginClasses)).append('\n');
        }
    ...
    /**
     * Splits a comma-separated package list into one bnd package glob per entry, trimming each entry so
     * that a list written as {@code pkg1, pkg2} does not produce a clause with a leading space.
     *
     * @param pluginClasses comma-separated package names, possibly null or empty
     * @return one glob per declared package, or a single {@code *} when no packages are declared
     */
    static List<String> toPackageGlobs(final String pluginClasses) {
        final List<String> globs = new ArrayList<>();
        if (pluginClasses != null) {
            for (final String packageName : pluginClasses.split(",")) {
        ...
        return globs.isEmpty() ? Collections.singletonList("*") : globs;
    }

    /**
     * Normalizes a comma-separated package list so the emitted manifest header has no stray whitespace.
     *
     * @param pluginClasses comma-separated package names
     * @return the same packages, trimmed and comma-joined without spaces
     */
    static String normalizePackageList(final String pluginClasses) {
        return Arrays.stream(pluginClasses.split(","))
                .map(String::trim)
