...
import static org.opensearch.dataprepper.plugins.lambda.processor.LambdaClientFactory.createLambdaClient;
...
class LambdaClientFactoryTest {
    @Mock
    private LambdaProcessorConfig lambdaSinkConfig;
    @Mock
    ...
    void createLambdaClient_with_real_LambdaClient() {
        ...
        final LambdaClient lambdaClient = createLambdaClient(lambdaSinkConfig, awsCredentialsSupplier);
    ...
    void createlambdaClient_provides_correct_inputs(final String regionString) {
        ...
        try (final MockedStatic<LambdaClient> lambdaClientMockedStatic = mockStatic(LambdaClient.class)) {
            lambdaClientMockedStatic.when(LambdaClient::builder).thenReturn(lambdaClientBuilder);
            createLambdaClient(lambdaSinkConfig, awsCredentialsSupplier);
        }
