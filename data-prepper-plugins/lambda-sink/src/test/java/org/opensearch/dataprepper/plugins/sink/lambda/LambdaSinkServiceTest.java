...
public class LambdaSinkServiceTest {
    ...
    @Test
    public void lambda_sink_test_with_request_response_invocation_success_push_to_lambda() throws IOException {
        final String config =
        ...
        verify(lambdaClient).invoke(invokeRequestCaptor.capture());
        final InvokeRequest actualRequest = invokeRequestCaptor.getValue();
        assertEquals(actualRequest.functionName(), "test_function");
        assertEquals(actualRequest.invocationType().toString(), "RequestResponse");
        verify(numberOfRecordsSuccessCounter).increment(records.size());
    }
    ...
    public void lambda_sink_test_batch_enabled() throws IOException {
        when(lambdaSinkConfig.getSync()).thenReturn(false);
