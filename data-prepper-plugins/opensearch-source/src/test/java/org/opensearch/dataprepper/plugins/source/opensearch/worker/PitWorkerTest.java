...
import static org.opensearch.dataprepper.plugins.source.opensearch.worker.PitWorker.DEFAULT_KEEP_ALIVE;
...
public class PitWorkerTest {
    ...
    @Test
    void run_with_getNextPartition_with_non_empty_partition_creates_and_deletes_pit_and_closes_that_partition() throws InterruptedException {
        ...
        assertThat(createPointInTimeRequest.getKeepAlive(), equalTo(DEFAULT_KEEP_ALIVE));
    ...
    @Test
    void run_with_getNextPartition_with_valid_existing_point_in_time_does_not_create_another_point_in_time() throws InterruptedException {
        final SourcePartition<OpenSearchIndexProgressState> sourcePartition = mock(SourcePartition.class);
        final String partitionKey = UUID.randomUUID().toString();
        when(sourcePartition.getPartitionKey()).thenReturn(partitionKey);
