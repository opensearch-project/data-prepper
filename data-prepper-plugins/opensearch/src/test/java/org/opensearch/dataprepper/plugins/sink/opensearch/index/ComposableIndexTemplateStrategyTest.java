...
class ComposableIndexTemplateStrategyTest {
    ...
    private ComposableIndexTemplateStrategy createObjectUnderTest() {
        return new ComposableIndexTemplateStrategy(openSearchClient);
    }

    @Test
    void getExistingTemplateVersion_should_calls_existIndexTemplate_with_indexTemplateName() throws IOException {
        ...
        final ArgumentCaptor<ExistsIndexTemplateRequest> existsTemplateRequestArgumentCaptor = ArgumentCaptor.forClass(ExistsIndexTemplateRequest.class);
        verify(openSearchIndicesClient).existsIndexTemplate(existsTemplateRequestArgumentCaptor.capture());

        final ExistsIndexTemplateRequest actualRequest = existsTemplateRequestArgumentCaptor.getValue();
        assertThat(actualRequest.name(), equalTo(indexTemplateName));

        verifyNoMoreInteractions(openSearchIndicesClient);
    }
