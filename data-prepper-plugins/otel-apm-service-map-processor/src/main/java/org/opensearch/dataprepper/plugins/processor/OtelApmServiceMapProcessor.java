...
import org.opensearch.dataprepper.plugins.processor.model.ServiceConnection;
import org.opensearch.dataprepper.plugins.processor.model.ServiceOperationDetail;
import org.opensearch.dataprepper.plugins.processor.model.Service;
import org.opensearch.dataprepper.plugins.processor.model.Operation;
...
import org.opensearch.dataprepper.plugins.processor.model.internal.ServerSpanDecoration;
import org.opensearch.dataprepper.plugins.processor.model.internal.ThreeWindowTraceData;
import org.opensearch.dataprepper.plugins.processor.model.internal.ThreeWindowTraceDataWithDecorations;
import org.opensearch.dataprepper.plugins.processor.model.internal.EphemeralSpanDecorations;
...
public class OtelApmServiceMapProcessor extends AbstractProcessor<Record<Event>, Record<Event>> implements RequiresPeerForwarding {
    ...
    private void processSpan(final Span span, final Map<byte[], Collection<SpanStateData>> batchStateData) {
        if (span.getServiceName() != null) {
            ...
            try {
                final byte[] traceId = Hex.decodeHex(span.getTraceId());
                ...
                Collection<SpanStateData> spansForTrace = batchStateData.computeIfAbsent(traceId,
                        k -> new HashSet<>());
    ...
    private Collection<Record<Event>> processCurrentWindowSpans() {
        ...
        final Map<byte[], Collection<SpanStateData>> currentSpansByTraceId = buildSpansByTraceIdMap(currentWindow);
        final Map<byte[], Collection<SpanStateData>> nextSpansByTraceId = buildSpansByTraceIdMap(nextWindow);

        for (byte[] traceId : currentSpansByTraceId.keySet()) {
            ...
            if (!traceData.processingSpans.isEmpty()) {
    ...
    private Instant getAnchorTimestampFromSpan(final SpanStateData spanStateData, final Instant fallbackTime) {
        ...
        try {
            if (spanStateData.endTime != null && !spanStateData.endTime.isEmpty()) {
                timestamp = Instant.parse(spanStateData.endTime);
            }
        } catch (Exception e) {
            LOG.debug("Failed to parse span endTime '{}', using fallback time: {}", 
                     spanStateData.endTime, e.getMessage());
        }
     ...
     */
    private Map<byte[], Collection<SpanStateData>> buildSpansByTraceIdMap(final MapDbProcessorState<Collection<SpanStateData>> window) {
        final Map<byte[], Collection<SpanStateData>> spansByTraceId = new HashMap<>();
     ...
     */
    private ThreeWindowTraceData buildThreeWindowTraceData(final byte[] traceId,
                                                           final Map<byte[], Collection<SpanStateData>> previousSpansByTraceId,
                                                           final Map<byte[], Collection<SpanStateData>> currentSpansByTraceId,
                                                           final Map<byte[], Collection<SpanStateData>> nextSpansByTraceId) {
        ...
        for (SpanStateData span : lookupSpans) {
            final String spanIdHex = Hex.encodeHexString(span.spanId);
            spansBySpanId.put(spanIdHex, span);

            if (span.parentSpanId != null) {
                final String parentSpanIdHex = Hex.encodeHexString(span.parentSpanId);
                childrenByParentId.computeIfAbsent(parentSpanIdHex, k -> new HashSet<>()).add(span);
            }
        }

        for (SpanStateData span : processingSpans) {
            processingSpanIds.add(Hex.encodeHexString(span.spanId));
        }
    ...
    private ThreeWindowTraceDataWithDecorations buildThreeWindowTraceDataWithDecorations(
        ...
        return new ThreeWindowTraceDataWithDecorations(
                baseTraceData.processingSpans,
                baseTraceData.lookupSpans,
                baseTraceData.spansBySpanId,
                baseTraceData.childrenByParentId,
                baseTraceData.processingSpanIds,
                decorations);
    }
    ...
    private void decorateClientSpansFirstPassWithEphemeralStorage(final ThreeWindowTraceDataWithDecorations traceData) {
        for (SpanStateData clientSpan : traceData.lookupSpans) {
            if (SPAN_KIND_CLIENT.equals(clientSpan.spanKind)) {
                final String clientSpanIdHex = clientSpan.getSpanIdHex();
                final Collection<SpanStateData> childServerSpans = traceData.childrenByParentId.getOrDefault(clientSpanIdHex, Collections.emptyList())
                        .stream()
                        .filter(span -> SPAN_KIND_SERVER.equals(span.spanKind))
                ...
                if (!childServerSpans.isEmpty()) {
                    ...
                    remoteService = childServerSpan.serviceName;
                    remoteOperation = childServerSpan.getOperationName();
                    remoteEnvironment = childServerSpan.getEnvironment();
                    remoteGroupByAttributes = childServerSpan.groupByAttributes;
                }
                ...
                traceData.decorations.setClientDecoration(clientSpanIdHex, decoration);
            }
        }
    }
    ...
    private void decorateServerSpansSecondPassWithEphemeralStorage(final ThreeWindowTraceDataWithDecorations traceData) {
        for (SpanStateData serverSpan : traceData.lookupSpans) {
            if (SPAN_KIND_SERVER.equals(serverSpan.spanKind)) {
                ...
                traceData.decorations.setServerDecoration(serverSpan.getSpanIdHex(), serverDecoration);

                for (SpanStateData clientSpan : clientDescendants) {
                    final String clientSpanIdHex = clientSpan.getSpanIdHex();
                    final ClientSpanDecoration existingDecoration = traceData.decorations.getClientDecoration(clientSpanIdHex);
                    
                    if (existingDecoration != null) {
                        final ClientSpanDecoration updatedDecoration = new ClientSpanDecoration(
                                serverSpan.getOperationName(),
                                existingDecoration.remoteEnvironment,
                                existingDecoration.remoteService,
                                existingDecoration.remoteOperation,
                                existingDecoration.remoteGroupByAttributes
                        );
                        traceData.decorations.setClientDecoration(clientSpanIdHex, updatedDecoration);
                    } else {
                        ...
                        traceData.decorations.setClientDecoration(clientSpanIdHex, newDecoration);
                    }
                }
            }
        }
    ...
    private Collection<Record<Event>> generateServiceConnectionsFromEphemeralDecorations(final ThreeWindowTraceDataWithDecorations traceData,
        ...
        for (SpanStateData clientSpan : traceData.processingSpans) {
            if (SPAN_KIND_CLIENT.equals(clientSpan.spanKind)) {
                final ClientSpanDecoration decoration = traceData.decorations.getClientDecoration(clientSpan.getSpanIdHex());
                
                if (decoration != null && !"unknown".equals(decoration.remoteService)) {
                    final Service clientService = new Service(
                            new Service.KeyAttributes(clientSpan.getEnvironment(), clientSpan.serviceName),
                            clientSpan.groupByAttributes
                    );

                    final Service serverService = new Service(
                            new Service.KeyAttributes(decoration.remoteEnvironment, decoration.remoteService),
                            decoration.remoteGroupByAttributes
                    );
                    ...
                    final Event connectionEvent = JacksonEvent.builder()
                            .withEventType(EVENT_TYPE_OTEL_APM_SERVICE_MAP)
    ...
    private Collection<Record<Event>> generateServiceOperationDetailsFromEphemeralDecorations(final ThreeWindowTraceDataWithDecorations traceData,
        ...
        for (SpanStateData serverSpan : traceData.processingSpans) {
            if (SPAN_KIND_SERVER.equals(serverSpan.spanKind)) {
                final ServerSpanDecoration decoration = traceData.decorations.getServerDecoration(serverSpan.getSpanIdHex());
                ...
                if (decoration != null && !decoration.clientDescendants.isEmpty()) {
                    for (SpanStateData clientSpan : decoration.clientDescendants) {
                        final ClientSpanDecoration clientDecoration = traceData.decorations.getClientDecoration(clientSpan.getSpanIdHex());
                        
                        if (clientDecoration != null) {
                            final Service service = new Service(
                                    new Service.KeyAttributes(serverSpan.getEnvironment(), serverSpan.serviceName),
                                    serverSpan.groupByAttributes
                            );

                            final Service remoteService = new Service(
                                    new Service.KeyAttributes(clientDecoration.remoteEnvironment, clientDecoration.remoteService),
                                    clientDecoration.remoteGroupByAttributes
                            );

                            final Operation operation = new Operation(
                                    serverSpan.getOperationName(),
                                    remoteService,
                                    clientDecoration.remoteOperation
                            );
                            ...
                            final Event operationEvent = JacksonEvent.builder()
                                    .withEventType(EVENT_TYPE_OTEL_APM_SERVICE_MAP)
    ...
    private Collection<SpanStateData> findClientDescendantsForServerThreeWindow(final SpanStateData serverSpan,
        ...
        final String serverSpanIdHex = Hex.encodeHexString(serverSpan.spanId);

        final Set<String> visited = new HashSet<>();
        final java.util.Queue<String> queue = new java.util.LinkedList<>();
        queue.offer(serverSpanIdHex);
        visited.add(serverSpanIdHex);

        while (!queue.isEmpty()) {
            final String currentSpanIdHex = queue.poll();
            final Collection<SpanStateData> children = traceData.childrenByParentId.getOrDefault(currentSpanIdHex, Collections.emptyList());

            for (SpanStateData child : children) {
                final String childSpanIdHex = Hex.encodeHexString(child.spanId);

                if (!visited.contains(childSpanIdHex)) {
                    visited.add(childSpanIdHex);

                    if (serverSpan.serviceName.equals(child.serviceName)) {
                        if (SPAN_KIND_CLIENT.equals(child.spanKind)) {
                            clientDescendants.add(child);
                        }

                        queue.offer(childSpanIdHex);
                    }
                }
            }
        }
