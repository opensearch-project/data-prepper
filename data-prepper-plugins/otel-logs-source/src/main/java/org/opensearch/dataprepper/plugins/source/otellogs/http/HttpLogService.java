...
public class HttpLogService {
    ...
    public HttpLogService(PluginMetrics pluginMetrics, OTelLogsSourceConfig oTelLogsSourceConfig, PluginFactory pluginFactory) {
    ...
    public ArmeriaHttpService create(ServerBuilder serverBuilder, Buffer<Record<Object>> buffer) {
        RetryInfoConfig retryInfo = oTelLogsSourceConfig.getRetryInfo() != null
                ...
                : DEFAULT_RETRY_INFO;
        ArmeriaHttpService httpService = new ArmeriaHttpService(buffer, pluginMetrics, oTelLogsSourceConfig.getRequestTimeoutInMillis());
    ...
    private void configureAuthentication(ServerBuilder serverBuilder) {
        if (oTelLogsSourceConfig.getAuthentication() == null || oTelLogsSourceConfig.getAuthentication().getPluginName().equals(UNAUTHENTICATED_PLUGIN_NAME)) {
            LOG.warn("Creating otel_logs_source http service without authentication. This is not secure.");
            LOG.warn("In order to set up Http Basic authentication for the otel-logs-source, go here: https://github.com/opensearch-project/data-prepper/tree/main/data-prepper-plugins/otel-trace-source#authentication-configurations");
        } else {
            ArmeriaHttpAuthenticationProvider authenticationProvider = createAuthenticationProvider(oTelLogsSourceConfig.getAuthentication());
