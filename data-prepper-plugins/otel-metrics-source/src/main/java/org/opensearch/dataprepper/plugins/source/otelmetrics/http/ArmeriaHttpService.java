...
public class ArmeriaHttpService {
    ...
    @ConsumesJson
    @ConsumesProtobuf
    public ExportMetricsServiceResponse exportMetrics(final ExportMetricsServiceRequest request) {
    ...
    private void processRequest(final ExportMetricsServiceRequest request) {
        if (buffer.isByteBuffer()) {
            ...
            final Collection<Record<? extends Metric>> metrics = parseRequest(request);
            writeRecords(request, metrics);
        }
    ...
    private Collection<Record<? extends Metric>> parseRequest(final ExportMetricsServiceRequest request) {
        try {
            final AtomicInteger droppedCounter = new AtomicInteger(0);
            return oTelProtoDecoder.parseExportMetricsServiceRequest(
                    request, droppedCounter, DEFAULT_EXPONENTIAL_HISTOGRAM_MAX_ALLOWED_SCALE,
                    Instant.now(), true, true, true);
        } catch (final Exception e) {
            LOG.warn(DataPrepperMarkers.SENSITIVE,
                    "Failed to parse the request with error '{}'. Request body: {}.", e.getMessage(), request);
            throw new BadRequestException(e.getMessage(), e);
        }
    }

    private void writeBytes(final ExportMetricsServiceRequest request) {
        try {
    ...
    private void writeRecords(final ExportMetricsServiceRequest request,
