...
public class HttpExceptionHandler implements ExceptionHandlerFunction {
    ...
    static final String ARMERIA_REQUEST_TIMEOUT_MESSAGE =
            "Timeout waiting for request to be served. This is usually due to the buffer being full.";
    ...
    public HttpExceptionHandler(final PluginMetrics pluginMetrics,
    ...
    public HttpResponse handleException(final ServiceRequestContext ctx,
        ...
        final StatusHolder statusHolder = createStatus(exceptionCause);

        try {
            final JsonFormat.TypeRegistry typeRegistry = JsonFormat.TypeRegistry.newBuilder()
                    .add(RetryInfo.getDescriptor())
                    .build();
            final JsonFormat.Printer printer = JsonFormat.printer().usingTypeRegistry(typeRegistry);
            return HttpResponse.of(statusHolder.getHttpStatus(), MediaType.JSON,
                    printer.print(statusHolder.getStatus()));
        } catch (InvalidProtocolBufferException ipbe) {
            throw new RuntimeException(ipbe);
        }
    }

    private StatusHolder createStatus(final Throwable e) {
        if (e instanceof RequestTimeoutException || e instanceof TimeoutException) {
            requestTimeoutsCounter.increment();
            return new StatusHolder(createStatus(e, Status.Code.RESOURCE_EXHAUSTED),
                    createHttpStatusFromProtoBufStatus(Status.Code.RESOURCE_EXHAUSTED));
            ...
            return new StatusHolder(createStatus(e, Status.Code.INVALID_ARGUMENT),
                    createHttpStatusFromProtoBufStatus(Status.Code.INVALID_ARGUMENT));
        } else if ((e instanceof StatusRuntimeException) &&
                (e.getMessage().contains("Invalid protobuf byte sequence") ||
                        e.getMessage().contains("Can't decode compressed frame"))) {
            badRequestsCounter.increment();
            return new StatusHolder(createStatus(e, Status.Code.INVALID_ARGUMENT),
                    createHttpStatusFromProtoBufStatus(Status.Code.INVALID_ARGUMENT));
        } else if (e instanceof RequestCancelledException) {
            requestTimeoutsCounter.increment();
            return new StatusHolder(createStatus(e, Status.Code.CANCELLED),
                    createHttpStatusFromProtoBufStatus(Status.Code.CANCELLED));
        } else {
            LOG.error("Unexpected exception handling http request", e);
            internalServerErrorCounter.increment();
            return new StatusHolder(createStatus(e, Status.Code.INTERNAL),
                    createHttpStatusFromProtoBufStatus(Status.Code.INTERNAL));
        }
    }

    private HttpStatus createHttpStatusFromProtoBufStatus(final Status.Code status) {
    ...
    private com.google.rpc.Status createStatus(final Throwable e, final Status.Code code) {
        final com.google.rpc.Status.Builder builder = com.google.rpc.Status.newBuilder().setCode(code.value());
    ...
    private static class StatusHolder {
        ...
        public StatusHolder(final com.google.rpc.Status status, final HttpStatus httpStatus) {
