...
public class OTelProtoStandardCodec {
    ...
    public static class OTelProtoDecoder implements OTelProtoCodec.OTelProtoDecoder {
        ...
        protected Stream<OpenTelemetryLog> parseResourceLogs(ResourceLogs rs, final Instant timeReceived) {
            ...
            final Map<String, Object> resourceAttributes = getResourceAttributes(rs.getResource(), rs.getSchemaUrl());
        ...
        protected List<Span> parseResourceSpans(final ResourceSpans resourceSpans, final Instant timeReceived) {
            ...
            final Map<String, Object> resourceAttributes = getResourceAttributes(resourceSpans.getResource(), resourceSpans.getSchemaUrl());
            if (!resourceSpans.getScopeSpansList().isEmpty()) {
        ...
        private List<Span> parseScopeSpans(final List<ScopeSpans> scopeSpansList, final String serviceName, final Map<String, Object> resourceAttributes, final Instant timeReceived) {
            return scopeSpansList.stream()
                    .map(scopeSpans -> parseSpans(scopeSpans.getSpansList(), scopeSpans.getScope(), scopeSpans.getSchemaUrl(),
        ...
        private <T> List<Span> parseSpans(final List<io.opentelemetry.proto.trace.v1.Span> spans, final T scope,
            ...
            return spans.stream()
                    .map(span -> {
                        ...
                        return parseSpan(span, scopeAttributes, schemaUrl, serviceName, resourceAttributes, timeReceived);
                    })
                    .collect(Collectors.toList());
        }
        ...
        protected Span parseSpan(final io.opentelemetry.proto.trace.v1.Span sp,
                    final Map<String, Object> instrumentationScopeAttributes,
                    final String schemaUrl,
                    final String serviceName,
                    final Map<String, Object> resourceAttributes,
                     final Instant timeReceived) {
        ...
        private Map<String, Object> getMetricMetadata(io.opentelemetry.proto.metrics.v1.Metric metric) {
            List<KeyValue> metadata = metric.getMetadataList();
            return convertKeyValueToAttributes(metadata);
        }
        ...
        private List<? extends Record<? extends Metric>> mapGauge(
            ...
            return metric.getGauge().getDataPointsList().stream()
                .map(dp -> JacksonStandardGauge.builder()
                        ...
                        .withScope(ils)
                        .withMetricMetadata(getMetricMetadata(metric))
                        .withResource(resourceAttributes)
            ...
            return metric.getExponentialHistogram()
                ...
                .map(dp -> {
                    JacksonExponentialHistogram.Builder builder = JacksonStandardExponentialHistogram.builder()
                            ...
                            .withZeroThreshold(dp.getZeroThreshold())
    ...
    public static class OTelProtoEncoder implements OTelProtoCodec.OTelProtoEncoder {
        ...
        public ResourceSpans convertToResourceSpans(final Span span) throws UnsupportedEncodingException, DecoderException {
            ...
            if (span.getResource() != null) {
                rsBuilder.setSchemaUrl((String)span.getResource().get(SCHEMA_URL_KEY));
            }
        ...
        public io.opentelemetry.proto.logs.v1.ResourceLogs convertToResourceLogs(final Log log) 
            ...
            if (log instanceof OpenTelemetryLog) {
                final OpenTelemetryLog otelLog = (OpenTelemetryLog) log;
                
                // Build resource from the log's resource map (same pattern as convertToResourceSpans)
                if (otelLog.getResource() != null) {
                    final Resource resource = constructResource(otelLog.getResource());
                    resourceLogsBuilder.setResource(resource);
                    final String schemaUrl = (String) otelLog.getResource().get(SCHEMA_URL_KEY);
                    if (schemaUrl != null) {
                        resourceLogsBuilder.setSchemaUrl(schemaUrl);
                    }
                }
                
                // Build scope logs with instrumentation scope
                final ScopeLogs.Builder scopeLogsBuilder = ScopeLogs.newBuilder();
                if (otelLog.getScope() != null) {
                    scopeLogsBuilder.setScope(constructInstrumentationScope(otelLog.getScope()));
                }
                if (otelLog.getSchemaUrl() != null) {
                    scopeLogsBuilder.setSchemaUrl(otelLog.getSchemaUrl());
                }
                
                scopeLogsBuilder.addLogRecords(constructLogRecord(otelLog));
                resourceLogsBuilder.addScopeLogs(scopeLogsBuilder.build());
            } else {
                // Fallback for plain Log instances
                final ScopeLogs.Builder scopeLogsBuilder = ScopeLogs.newBuilder();
                scopeLogsBuilder.addLogRecords(constructLogRecord(log));
                resourceLogsBuilder.addScopeLogs(scopeLogsBuilder.build());
            }
        ...
        protected LogRecord constructLogRecord(final Log log) throws UnsupportedEncodingException, DecoderException {
            ...
            if (log instanceof OpenTelemetryLog) {
                ...
                // Set timestamps
                if (otelLog.getTime() != null) {
                    builder.setTimeUnixNano(convertISO8601ToNanos(otelLog.getTime()));
                }
                if (otelLog.getObservedTime() != null) {
                    builder.setObservedTimeUnixNano(convertISO8601ToNanos(otelLog.getObservedTime()));
                }
                
                // Set body
                if (otelLog.getBody() != null) {
                    builder.setBody(objectToAnyValue(otelLog.getBody()));
                }
                ...
                // Set trace context
                if (otelLog.getTraceId() != null && !otelLog.getTraceId().isEmpty()) {
                    builder.setTraceId(ByteString.copyFrom(Hex.decodeHex(otelLog.getTraceId())));
                }
                if (otelLog.getSpanId() != null && !otelLog.getSpanId().isEmpty()) {
                    builder.setSpanId(ByteString.copyFrom(Hex.decodeHex(otelLog.getSpanId())));
                }
                
                // Set flags
                if (otelLog.getFlags() != null) {
                    builder.setFlags(otelLog.getFlags());
                }
                
                // Set dropped attributes count
                if (otelLog.getDroppedAttributesCount() != null) {
                    builder.setDroppedAttributesCount(otelLog.getDroppedAttributesCount());
                }
