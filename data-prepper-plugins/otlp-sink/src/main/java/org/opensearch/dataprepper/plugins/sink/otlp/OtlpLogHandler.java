 ...
 */
public class OtlpLogHandler implements OtlpSignalHandler<ResourceLogs> {
    ...
    @Override
    public ResourceLogs encodeEvent(final Event event) throws Exception {
        if (!(event instanceof Log)) {
            throw new IllegalArgumentException("OtlpLogHandler expects a Log event but received: " + event.getClass().getName());
        }
        return encoder.convertToResourceLogs((Log) event);
    }

    @Override
    public long getSerializedSize(final ResourceLogs encodedData) {
        return encodedData.getSerializedSize();
    }

    @Override
    public byte[] buildRequestPayload(final List<Pair<ResourceLogs, EventHandle>> batch) {
