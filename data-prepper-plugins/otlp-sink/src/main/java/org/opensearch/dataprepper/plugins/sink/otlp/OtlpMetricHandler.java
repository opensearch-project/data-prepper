 ...
 */
public class OtlpMetricHandler implements OtlpSignalHandler<ResourceMetrics> {
    ...
    @Override
    public ResourceMetrics encodeEvent(final Event event) throws Exception {
        if (!(event instanceof Metric)) {
            throw new IllegalArgumentException("OtlpMetricHandler expects a Metric event but received: " + event.getClass().getName());
        }
        return encoder.convertToResourceMetrics((Metric) event);
    }

    @Override
    public long getSerializedSize(final ResourceMetrics encodedData) {
        return encodedData.getSerializedSize();
    }

    @Override
    public byte[] buildRequestPayload(final List<Pair<ResourceMetrics, EventHandle>> batch) {
