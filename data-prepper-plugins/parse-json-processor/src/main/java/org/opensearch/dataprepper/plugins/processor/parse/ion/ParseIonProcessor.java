...
public class ParseIonProcessor extends AbstractParseProcessor {
    ...
    protected Optional<Object> readValue(String message, Event context) {
        try {
            ...
            final Object value = objectMapper.convertValue(objectMapper.readValue(message, new TypeReference<>() {}), new TypeReference<Object>() {});
            if (depth == 0 || !(value instanceof Map)) {
                return Optional.of(value);
            }
            return Optional.of(objectMapper.convertValue(convertNestedObjectToString((Map<String, Object>) value, 1, depth), new TypeReference<>() {}));
