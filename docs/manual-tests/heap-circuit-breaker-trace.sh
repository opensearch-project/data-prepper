...
# =============================================================================
# heap-circuit-breaker-trace.sh
#
# Trace-source-focused variant of heap-circuit-breaker.sh.
#
# Boots a single-node OpenSearch (Docker) + a locally-built Data Prepper with
# a TRACE-ONLY pipeline, floods the gRPC entry point with telemetrygen, and
# asserts that the heap circuit breaker engages on BOTH the gRPC and HTTP
# entry points of the OTel trace source.
#
# The trace source is special because OTelTraceSource registers two services
# on the same Armeria ServerBuilder:
#   1. the gRPC TraceService (hit by telemetrygen / OTLP-gRPC clients)
#   2. the additional HTTP TraceService (hit by OTLP-HTTP/JSON clients)
#
...
#
# Usage:
#   ./heap-circuit-breaker-trace.sh [--skip-build] [--keep-running] [--cleanup] [--help]
#
# Flags:
...
# ---------- Constants ----------
readonly CONTAINER_NAME="cb-test-trace-os"
...
readonly DP_LOG="/tmp/cb-test-trace-dp.log"
readonly DP_PID="/tmp/cb-test-trace-dp.pid"
readonly TELEM_LOG="/tmp/cb-test-trace-telemetrygen.log"
readonly TRIP_TELEM_LOG="/tmp/cb-test-trace-telemetrygen-trip.log"
readonly HTTP_PROBE_LOG="/tmp/cb-test-trace-http-probes.log"
readonly HTTP_PROBE_STOP="/tmp/cb-test-trace-http-probe.stop"

# OTLP-HTTP/JSON entry point exposed by the trace source's additional HTTP
# service (see ArmeriaHttpService#exportTrace). Same port as the gRPC service.
readonly OTLP_HTTP_PATH="/opentelemetry.proto.collector.trace.v1.TraceService/Export"
# Minimal valid ExportTraceServiceRequest. The circuit-breaker decorator runs
...
# Send a single OTLP-HTTP/JSON trace probe. Echoes the HTTP status code (or
...
http_probe_once() {
    curl -s -o /dev/null -w "%{http_code}" \
        ...
        -d "$OTLP_PROBE_JSON" \
        "http://localhost:${OTEL_TRACE_PORT}${OTLP_HTTP_PATH}" 2>/dev/null || echo "000"
}
...
phase_write_configs() {
    section "Writing TRACE-ONLY configs into $DP_HOME/{config,pipelines}/"
...
# Tuned for -Xmx256m with a single trace pipeline.
# 200mb / 150mb gives a comfortable hysteresis band well above the idle
# footprint (~60mb for one trace pipeline + OpenSearch sink) so the breaker
...
YAML
    ...
    # Trace-only pipeline. We deliberately do NOT register metrics_pipeline or
    # logs_pipeline because we want all heap pressure (and all breaker activity)
    # to be attributable to the OTel trace source.
...
logger.oteltrace.level = info
PROPS
    log "Configs written (trace-only pipeline)."
}
...
phase_start_dataprepper() {
    ...
    # Proof point specific to this script: confirm the trace source wired the
    # circuit-breaker HTTP decorator (CreateServer/OTelTraceSource logs this
    # on startup when a CircuitBreaker bean is present).
    if grep -q "Installing circuit-breaker HTTP decorator for otel_trace_source" "$DP_LOG"; then
        pass "Trace source installed the circuit-breaker HTTP decorator at startup."
    else
        warn "Did not see 'Installing circuit-breaker HTTP decorator for otel_trace_source' in DP log — the HTTP-path proof below may fail."
    fi
}
...
phase_baseline() {
    ...
    # gRPC warm-up via telemetrygen (matches the original script's warm-up).
...
phase_trip() {
    ...
    # that the server-level decorator on the trace source's ServerBuilder
    ...
    # Background: gRPC flood with telemetrygen.
    log "Starting telemetrygen gRPC flood (workers=$LOAD_WORKERS rate=$LOAD_RATE duration=${LOAD_DURATION}s)"
    ...
    if (( n_503 > 0 )); then
        pass "HTTP path returned 503 $n_503 time(s) during the open window — P1 HTTP decorator engaged on the trace source's additional HTTP service."
    else
        fail "HTTP path returned 0 instances of 503 across $((n_200 + n_503 + n_other)) probe(s) — the HTTP decorator is NOT rejecting requests on the trace source's HTTP entry point."
    fi
    if (( n_other > 0 )); then
