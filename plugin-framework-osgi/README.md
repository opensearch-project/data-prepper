...
| `BundleAdapter` | Wraps legacy plugin JARs as OSGi bundles without code changes |
| `StaticBundleLoader` | Installs, resolves, and starts bundles; emits Micrometer metrics |
...
| `DataPrepperApiVersion` | Manages API compatibility versioning for bundles |
...
| `DataPrepperOsgiPackages` | Defines the set of Data Prepper packages exported to the OSGi framework |
...
-Dplugin.framework=legacy

# OSGi mode - uses Felix framework
-Dplugin.framework=osgi

# Directory containing plugin JARs (required in OSGi mode)
-Dplugin.bundles.dir=/path/to/plugin/bundles
```

The `plugin.bundles.dir` property tells the OSGi framework where to find plugin JARs. If not set, the framework starts with no plugin bundles and only classpath fallback is available.

## Rollback Procedure

1. Set `-Dplugin.framework=legacy` (or remove the property entirely)
...
Existing plugins using `@DataPrepperPlugin` annotation and `data-prepper.plugins.properties` work in both modes without modification. The `BundleAdapter` automatically wraps legacy JARs as OSGi bundles.
