...
final class DataPrepperOsgiPackages {
     ...
     * {@code DataPrepperOsgiPlugin} under {@code data-prepper-gradle-plugins/osgi-plugin}. That
     * project is an included (composite) build, so neither side can depend on the other; the host
     * and the build-time manifest generator must agree on version normalization, so change both
     * together.
     */
