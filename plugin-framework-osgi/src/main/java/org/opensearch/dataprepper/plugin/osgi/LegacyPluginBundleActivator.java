 ...
 * <p>
 * This activator is the only place a plugin class is loaded for discovery purposes;
 * {@link OsgiPluginRegistry} reads the services published here rather than re-scanning
 * the bundle's annotations.
 */
public class LegacyPluginBundleActivator implements BundleActivator {
    ...
    private void registerPluginService(final BundleContext context, final Class<?> pluginClass,
        ...
        final String[] pluginNames = collectPluginNames(annotation);
        final Dictionary<String, Object> properties = new Hashtable<>();
        properties.put(OsgiPluginRegistry.PLUGIN_NAME_PROPERTY, pluginNames);
        properties.put(OsgiPluginRegistry.PLUGIN_TYPE_PROPERTY, annotation.pluginType().getName());
        properties.put(OsgiPluginRegistry.PLUGIN_CLASS_PROPERTY, pluginClass.getName());
        ...
        // type interface (OSGi would reject it). OsgiPluginRegistry finds these services
        // with an LDAP filter on PLUGIN_NAME_PROPERTY and reads the plugin type and class
        // from the service properties.
        final ServiceRegistration<?> registration = context.registerService(
                Class.class.getName(), pluginClass, properties);
        registrations.add(registration);

        LOG.debug("Registered OSGi service for plugin names {} (type: {}, class: {})",
                Arrays.toString(pluginNames), annotation.pluginType().getSimpleName(), pluginClass.getName());
    }

    /**
     * Collects every name the plugin answers to: its name plus any deprecated or alternate names.
     * These are published as a single multi-valued service property so that the service registry is
     * a complete description of the plugin and consumers need not re-read the annotation.
     *
     * @param annotation the plugin's annotation
     * @return all names for the plugin, with the primary name first
     */
    static String[] collectPluginNames(final DataPrepperPlugin annotation) {
        final List<String> names = new ArrayList<>();
        names.add(annotation.name());
        if (!DEFAULT_DEPRECATED_NAME.equals(annotation.deprecatedName())) {
            names.add(annotation.deprecatedName());
        }
        for (final String alternateName : annotation.alternateNames()) {
            if (!DEFAULT_ALTERNATE_NAME.equals(alternateName)) {
