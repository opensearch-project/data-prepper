...
public class PluginHotLoader {
    ...
    public PluginHotLoader(final BundleContext bundleContext) {
     ...
     * The JAR must already be a valid OSGi bundle (with Bundle-SymbolicName).
     *
    ...
    public String installPlugin(final File jarFile) throws IOException, BundleException {
        ...
        if (bundle.getSymbolicName() == null) {
            bundle.uninstall();
            throw new BundleException("JAR '" + jarFile.getName()
                    + "' is not an OSGi bundle (missing Bundle-SymbolicName).");
        }
        bundle.start();
