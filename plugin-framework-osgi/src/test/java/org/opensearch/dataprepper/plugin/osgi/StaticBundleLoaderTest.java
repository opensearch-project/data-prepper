...
class StaticBundleLoaderTest {
    ...
    void setUp() throws BundleException {
        ...
        final BundleAdapter adapter = new BundleAdapter(felixPluginManager.getBundleContext());
        loader = new StaticBundleLoader(felixPluginManager.getBundleContext(), adapter, meterRegistry);
    }
    ...
    @Test
    void loadBundles_with_legacy_jar_adapts_and_installs_successfully() throws Exception {
        ...
        assertThat(result.size(), is(1));
        assertThat(result.get(0).getState(), is(Bundle.ACTIVE));
        assertThat(result.get(0).getSymbolicName(), is(notNullValue()));
        // Adapted bundles get a generated symbolic name with the "org.opensearch.dataprepper.plugin." prefix
        assertThat(result.get(0).getSymbolicName(), containsString("org.opensearch.dataprepper.plugin."));
    }

    @Test
    void loadBundles_with_osgi_jar_installs_directly_without_adaptation() throws Exception {
    ...
    @Test
    void loadBundles_with_mixed_legacy_and_osgi_jars_installs_both() throws Exception {
        final File legacyJar = createLegacyJar("mixed-legacy");
        final File osgiJar = createValidBundle("mixed-osgi", "1.0.0");

        final List<Bundle> result = loader.loadBundles(Arrays.asList(legacyJar, osgiJar));
